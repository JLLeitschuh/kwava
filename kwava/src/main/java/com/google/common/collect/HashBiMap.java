...
public final class HashBiMap<K, V> extends IteratorBasedAbstractMap<K, V>
  ...
  private void init(int expectedSize) {
    ...
    int tableSize = Hashing.INSTANCE.closedTableSize(expectedSize, LOAD_FACTOR);
  ...
  public boolean containsKey( Object key) {
    return seekByKey(key, INSTANCE.smearedHash(key)) != null;
  }

  @Override
  public boolean containsValue( Object value) {
    return seekByValue(value, INSTANCE.smearedHash(value)) != null;
  }


  @Override
  public V get( Object key) {
    return Maps.valueOrNull(seekByKey(key, INSTANCE.smearedHash(key)));
  }
  ...
  private V put( K key,  V value, boolean force) {
    int keyHash = INSTANCE.smearedHash(key);
    int valueHash = INSTANCE.smearedHash(value);
  ...
  private void rehashIfNecessary() {
    ...
    if (Hashing.INSTANCE.needsResizing(size, oldKToV.length, LOAD_FACTOR)) {
  ...
  public V remove( Object key) {
    BiEntry<K, V> entry = seekByKey(key, INSTANCE.smearedHash(key));
    if (entry == null) {
      return null;
    } else {
      delete(entry);
  ...
  private final class KeySet extends Maps.KeySet<K, V> {
    ...
    public boolean remove( Object o) {
      BiEntry<K, V> entry = seekByKey(o, INSTANCE.smearedHash(o));
      if (entry == null) {
        return false;
      } else {
        delete(entry);
  ...
  private final class Inverse extends IteratorBasedAbstractMap<V, K>
    ...
    public K get( Object value) {
      return Maps.keyOrNull(seekByValue(value, INSTANCE.smearedHash(value)));
    }
    ...
    public K remove( Object value) {
      BiEntry<K, V> entry = seekByValue(value, INSTANCE.smearedHash(value));
      if (entry == null) {
        return null;
      } else {
        delete(entry);
    ...
    private final class InverseKeySet extends Maps.KeySet<V, K> {
      ...
      @Override
      public boolean remove( Object o) {
        BiEntry<K, V> entry = seekByValue(o, INSTANCE.smearedHash(o));
        if (entry == null) {
          return false;
        } else {
          delete(entry);
