...
public final class Maps {
  ...
  static int capacity(int expectedSize) {
    ...
    if (expectedSize < Ints.INSTANCE.getMAX_POWER_OF_TWO()) {
  ...
  public static <K, V> MapDifference<K, V> difference(
    ...
    Preconditions.INSTANCE.checkNotNull(valueEquivalence);
  ...
  private static final class NavigableAsMapView<K, V> extends AbstractNavigableMap<K, V> {
    ...
    Spliterator<Entry<K, V>> entrySpliterator() {
      return CollectSpliterators.INSTANCE.map(set.spliterator(), e -> immutableEntry(e, function.apply(e)));
    }
  ...
  static class UnmodifiableEntrySet<K, V> extends UnmodifiableEntries<K, V>
    ...
    @Override
    public boolean equals( Object object) {
      return Sets.INSTANCE.equalsImpl(this, object);
    }

    @Override
    public int hashCode() {
      return Sets.INSTANCE.hashCodeImpl(this);
    }
  }
  ...
  static class TransformedEntriesMap<K, V1, V2> extends IteratorBasedAbstractMap<K, V2> {
    ...
    Spliterator<Entry<K, V2>> entrySpliterator() {
      return CollectSpliterators.INSTANCE.map(
  ...
  private static class FilteredKeyMap<K, V> extends AbstractFilteredMap<K, V> {
    ...
    @Override
    protected Set<Entry<K, V>> createEntrySet() {
      return Sets.INSTANCE.filter(unfiltered.entrySet(), predicate);
    }

    @Override
    Set<K> createKeySet() {
      return Sets.INSTANCE.filter(unfiltered.keySet(), keyPredicate);
    }
  ...
  static class FilteredEntryMap<K, V> extends AbstractFilteredMap<K, V> {
    ...
    FilteredEntryMap(Map<K, V> unfiltered, Predicate<? super Entry<K, V>> entryPredicate) {
      super(unfiltered, entryPredicate);
      filteredEntrySet = Sets.INSTANCE.filter(unfiltered.entrySet(), predicate);
    }
  ...
  static class UnmodifiableNavigableMap<K, V> extends ForwardingSortedMap<K, V>
    ...
    @Override
    public NavigableSet<K> navigableKeySet() {
      return Sets.INSTANCE.unmodifiableNavigableSet(delegate.navigableKeySet());
    }

    @Override
    public NavigableSet<K> descendingKeySet() {
      return Sets.INSTANCE.unmodifiableNavigableSet(delegate.descendingKeySet());
    }
  ...
  static class Values<K, V> extends AbstractCollection<V> {
    ...
    public boolean removeAll(Collection<?> c) {
      try {
        ...
        Set<K> toRemove = Sets.INSTANCE.newHashSet();
        for (Entry<K, V> entry : map().entrySet()) {
    ...
    public boolean retainAll(Collection<?> c) {
      try {
        ...
        Set<K> toRetain = Sets.INSTANCE.newHashSet();
        for (Entry<K, V> entry : map().entrySet()) {
          if (c.contains(entry.getValue())) {
  ...
  abstract static class EntrySet<K, V> extends Sets.ImprovedAbstractSet<Entry<K, V>> {
    ...
    public boolean removeAll(Collection<?> c) {
      try {
        ...
        return Sets.INSTANCE.removeAllImpl(this, c.iterator());
      }
    }
    ...
    public boolean retainAll(Collection<?> c) {
      try {
        ...
        // if the iterators don't support remove
        Set<Object> keys = Sets.INSTANCE.newHashSetWithExpectedSize(c.size());
        for (Object o : c) {
          if (contains(o)) {
            Entry<?, ?> entry = (Entry<?, ?>) o;
