...
class CompactHashMap<K, V> extends AbstractMap<K, V> implements Serializable {
  ...
  void init(int expectedSize, float loadFactor) {
    Preconditions.INSTANCE.checkArgument(expectedSize >= 0, "Initial capacity must be non-negative");
    Preconditions.INSTANCE.checkArgument(loadFactor > 0, "Illegal load factor");
    int buckets = Hashing.INSTANCE.closedTableSize(expectedSize, loadFactor);
  ...
  public V put( K key,  V value) {
    ...
    int hash = INSTANCE.smearedHash(key);
  ...
  public V remove( Object key) {
    return remove(key, INSTANCE.smearedHash(key));
  }
  ...
  class KeySetView extends Maps.KeySet<K, V> {
    ...
    public Object[] toArray() {
      return ObjectArrays.INSTANCE.copyAsObjectArray(keys, 0, size);
    }

    @Override
    public <T> T[] toArray(T[] a) {
      return ObjectArrays.INSTANCE.toArrayImpl(keys, 0, size, a);
    }
  ...
  class EntrySetView extends Maps.EntrySet<K, V> {
    ...
    public Spliterator<Entry<K, V>> spliterator() {
      return CollectSpliterators.INSTANCE.indexed(
  ...
  class ValuesView extends Maps.Values<K, V> {
    ...
    @Override
    public Object[] toArray() {
      return ObjectArrays.INSTANCE.copyAsObjectArray(values, 0, size);
    }

    @Override
    public <T> T[] toArray(T[] a) {
      return ObjectArrays.INSTANCE.toArrayImpl(values, 0, size, a);
    }
  }
