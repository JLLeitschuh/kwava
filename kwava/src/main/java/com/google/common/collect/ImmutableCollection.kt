...
class ImmutableCollection<E> internal constructor() : AbstractCollection<E>(), Serializable {
    /**
     ...
     * memory leaks.
     */
    internal abstract val isPartialView: Boolean
     ...
     * @since 2.0
     */
    open fun asList(): ImmutableList<E> {
        when (size) {
            0 -> return ImmutableList.of()
            1 -> return ImmutableList.of(iterator().next())
            else -> return RegularImmutableAsList<E>(this, toArray())
        }
    }
     ...
     */
    internal open fun copyIntoArray(dst: Array<Any?>, offset: Int): Int {
    ...
    internal open fun writeReplace(): Any {
        // We serialize by default to ImmutableList, the simplest thing that works.
        return ImmutableList.SerializedForm(toArray())
    }
     ...
     * @since 10.0
     */
    abstract class Builder<E> internal constructor() {
        /**
         ...
         */
        abstract fun add(element: E): Builder<E>
         ...
         */
        open fun add(vararg elements: E): Builder<E> {
         ...
         * @throws NullPointerException if {@code elements} is null or contains a null element
         */
        open fun addAll(elements: Iterable<out E>): Builder<E> {
            for (element in elements) {
                add(element)
            }
            return this
         ...
         * @throws NullPointerException if {@code elements} is null or contains a null element
         */
        open fun addAll(elements: Iterator<out E>): Builder<E> {
         ...
         */
        abstract fun build(): ImmutableCollection<E>

        companion object {
            internal val DEFAULT_INITIAL_CAPACITY = 4
            internal fun expandedCapacity(oldCapacity: Int, minCapacity: Int): Int {
                if (minCapacity < 0) {
                    throw AssertionError("cannot store more than MAX_VALUE elements")
                }
                // careful of overflow!
                var newCapacity = oldCapacity + (oldCapacity shr 1) + 1
                if (newCapacity < minCapacity) {
                    newCapacity = Integer.highestOneBit(minCapacity - 1) shl 1
                }
                if (newCapacity < 0) {
