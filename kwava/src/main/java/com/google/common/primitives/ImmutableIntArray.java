...
public final class ImmutableIntArray implements Serializable {
  ...
  public static ImmutableIntArray copyOf(Collection<Integer> values) {
    return values.isEmpty() ? EMPTY : new ImmutableIntArray(Ints.INSTANCE.toArray(values));
  }
  ...
  public static final class Builder {
    ...
    public Builder addAll(IntStream stream) {
      ...
      if (size > 0) { // known *and* nonempty
        ensureRoomFor(Ints.INSTANCE.saturatedCast(size));
      }
  ...
  public int get(int index) {
    Preconditions.INSTANCE.checkElementIndex(index, length());
  ...
  public ImmutableIntArray subArray(int startIndex, int endIndex) {
    Preconditions.INSTANCE.checkPositionIndexes(startIndex, endIndex, length());
  ...
  public int hashCode() {
    ...
    for (int i = start; i < end; i++) {
      hash *= 31;
      hash += Ints.INSTANCE.hashCode(array[i]);
    }
    return hash;
  }
