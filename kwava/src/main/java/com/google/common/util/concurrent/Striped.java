...
public abstract class Striped<L> {
  ...
  private abstract static class PowerOfTwoStriped<L> extends Striped<L> {
    ...
    PowerOfTwoStriped(int stripes) {
      Preconditions.INSTANCE.checkArgument(stripes > 0, "Stripes must be positive");
      this.mask = stripes > Ints.INSTANCE.getMAX_POWER_OF_TWO() ? ALL_SET : ceilToPowerOfTwo(stripes) - 1;
    }
  ...
  private static class CompactStriped<L> extends PowerOfTwoStriped<L> {
    ...
    private CompactStriped(int stripes, Supplier<L> supplier) {
      super(stripes);
      Preconditions.INSTANCE.checkArgument(stripes <= Ints.INSTANCE.getMAX_POWER_OF_TWO(), "Stripes must be <= 2^30)");
  ...
  static class SmallLazyStriped<L> extends PowerOfTwoStriped<L> {
    ...
    public L getAt(int index) {
      if (size != Integer.MAX_VALUE) {
        Preconditions.INSTANCE.checkElementIndex(index, size());
