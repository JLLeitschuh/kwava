...
class RegularImmutableBiMap<K, V> extends ImmutableBiMap<K, V> {
  ...
  static <K, V> ImmutableBiMap<K, V> fromEntryArray(int n, Entry<K, V>[] entryArray) {
    ...
    int tableSize = Hashing.INSTANCE.closedTableSize(n, MAX_LOAD_FACTOR);
    ...
    for (int i = 0; i < n; i++) {
      ...
      int keyBucket = Hashing.INSTANCE.smear(keyHash) & mask;
      int valueBucket = Hashing.INSTANCE.smear(valueHash) & mask;
  ...
  ImmutableSet<Entry<K, V>> createEntrySet() {
    return isEmpty()
        ? ImmutableSet.Companion.<Entry<K, V>>of()
  ...
  @LazyInit  private transient ImmutableBiMap<V, K> inverse;
  ...
  private final class Inverse extends ImmutableBiMap<V, K> {
    ...
    public K get( Object value) {
      ...
      int bucket = Hashing.INSTANCE.smear(value.hashCode()) & mask;
