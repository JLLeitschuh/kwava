...
public final class ImmutableRangeSet<C extends Comparable> extends AbstractRangeSet<C>
  ...
  public ImmutableSet<Range<C>> asRanges() {
    if (ranges.isEmpty()) {
      return ImmutableSet.Companion.of();
    }
  ...
  private final class AsSet extends ImmutableSortedSet<C> {
    ...
    public int size() {
      ...
      if (result == null) {
        ...
        result = size = Ints.INSTANCE.saturatedCast(total);
      }
    ...
    ImmutableSortedSet<C> headSetImpl(C toElement, boolean inclusive) {
      return subSet(Range.upTo(toElement, BoundType.Companion.forBoolean(inclusive)));
    }
    ...
    ImmutableSortedSet<C> subSetImpl(
      ...
      return subSet(
          Range.range(
              fromElement, BoundType.Companion.forBoolean(fromInclusive),
              toElement, BoundType.Companion.forBoolean(toInclusive)));
    }

    @Override
    ImmutableSortedSet<C> tailSetImpl(C fromElement, boolean inclusive) {
      return subSet(Range.downTo(fromElement, BoundType.Companion.forBoolean(inclusive)));
    }
    ...
    int indexOf(Object target) {
      if (contains(target)) {
        ...
        for (Range<C> range : ranges) {
          if (range.contains(c)) {
            return Ints.INSTANCE.saturatedCast(total + ContiguousSet.create(range, domain).indexOf(c));
          } else {
            total += ContiguousSet.create(range, domain).size();
          }
        }
